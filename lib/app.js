...
btnImportBpmn.addEventListener("click", () => {
  clearError();
  // `importBPMN` 内で確認ダイアログが表示されるため、ここでは不要
  // ただし、ボタンクリック時の確認はUXとして残すことも検討できる
  // 今回は `importBPMN` 内の確認に統一
  fileInputBpmn.click();
});
...
fileInputBpmn.addEventListener("change", (event) => {
  ...
  if (file && modeler) {
    ...
    reader.onload = async (e) => {
      // `importBPMN` にファイル名を渡して `currentFileNameForTitle` と `document.title` を更新
...
btnShowSvg.addEventListener("click", async () => {
  ...
  try {
    ...
    const processName =
      ...
      "BPMN Diagram"; // これは内部のダイアグラム名

    // SVG内部の <title> および <text> 要素は `processName` を使用
    // ブラウザタブのタイトルは `currentFileNameForTitle` を使用
    ...
    if (svgWindow) {
      ...
      svgWindow.document.title = currentFileNameForTitle; // SVG表示タブのタイトルには保存/読み込み時のファイル名を使用
...
btnExportSvg.addEventListener("click", async () => {
  ...
  try {
    ...
    // SVGエクスポート時に currentFileNameForTitle を更新 (次のSVG表示のため)
    // メインの document.title はBPMNファイル名を反映するため、ここでは変更しない
...
document.addEventListener("keydown", function (event) {
  ...
  if (event.ctrlKey && (event.key === "s" || event.key === "S")) {
    event.preventDefault();
    if (btnExportBpmn) {
      btnExportBpmn.click(); // エクスポートボタンのクリックイベントを発火
    }
  }
  // Ctrl+X: やり直し
  ...
  if (
    ...
    // console.log("元に戻すを実行しました。"); // bpmn-jsが処理
  }
});
